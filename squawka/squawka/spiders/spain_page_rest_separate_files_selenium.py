...
class MySpider(Spider):
    name = 'spain_page_rest_separate_files'
    url = 'http://www.squawka.com/match-results'
    pages = [10, 11]
    input_rows = {"page_9": 4, "page_10": 4, "page_11": 4, "page_12": 4, "page_13": 4}
    match_links = list()
    ...
    def start_requests(self):
        def return_match_list_script(page):
            match_list_script = """
            function main(splash, args)
              ...
              assert(splash:go("http://www.squawka.com/match-results?pg="""+str(page)+""""))
        ...
        for page in self.pages:
            match_list_script = return_match_list_script(page)
            yield Request(self.url, self.parse_match_list, meta = {
                'splash': {
                    'args': {
    ...
    def parse_match_list(self, response):
        ...
        """   
        print(len(self.match_links))
        if len(self.match_links) > 50:
    ...
    def parse_match_xml(self, response):
        ...
        full_date = processed_xml.find("kickoff").get_text()
        print(full_date)
        #setting up the main data container
        
        data = {
            'date': months[full_date.split(" ")[2]]+"/"+full_date.split(" ")[1]+"/"+full_date.split(" ")[3], #match date
        ...
        squawka_datasheet = spreadsheet("C:/Users/HP10/AppData/Local/Programs/Python/Python35/Lib/site-packages/scrapy/projects/squawka/squawka/excel/squawka_data_"+response.meta.get("page")+".xlsx", "Spain")     
                        ...
                        for event in data[event_name][detailed]: #{'mins':'25', 'type': 'Possession', ...}
                            ...
                            for field_name in field_names:
                                if field_name in event.keys():    
                                    squawka_datasheet.set_data(str(event[field_name]), "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment) #inputting 1 value at a time
                                    #if event_name == "interceptions":
                                    print(field_name+": "+str(event[field_name]))
                                elif field_name == "total":
                                    squawka_datasheet.set_data(data[event_name][detailed[:4]+"_short"], "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment)
                                    #if event_name == "interceptions":
                                    print(field_name+": "+str(data[event_name][detailed[:4]+"_short"]))
                                elif field_name == "event":
                                    squawka_datasheet.set_data(event_name, "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment)
                                    #if event_name == "interceptions":
                                    print(field_name+": "+str(event_name))
                                elif field_name == "team":
                                    squawka_datasheet.set_data(data[detailed[:4]], "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment)
                                    #if event_name == "interceptions":
                                    print(field_name+": "+str(data[detailed[:4]]))
                                elif field_name in ["home", "away", "date"]:
                                    squawka_datasheet.set_data(data[field_name], "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment)
                                    #if event_name == "interceptions":
                                    print(field_name+": "+str(data[field_name]))
                                else:
                                    squawka_datasheet.set_data("-", "Spain", self.input_rows["page_"+response.meta.get("page")], self.first_fields_column+column_increment)
                                    #if event_name == "interceptions":
                                    print(str(field_name)+": -")
                                column_increment += 1
                            self.input_rows["page_"+response.meta.get("page")] += 1
