...
class MySpider(Spider):
    ...
    def parse_match_list(self, response):
        ...
        """           
        print(self.match_links)
        for match_link in self.match_links[7:]:
            #self.logger.info("The link is: %s", match_link)
            yield SplashRequest(match_link, self.parse_match_xml,
                args={
    ...
    def parse_match_xml(self, response):
        ...
        yield scrapy.Request(url=link, callback=self.input_match, meta={'download_timeout': 5})
        
    def input_match(self, response):
        '''openpyxl API:
        ...
        player_ids = {}
        team_ids = {}        
        processed_xml = BeautifulSoup(response.body.decode("utf-8"), "xml")
